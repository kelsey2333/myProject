 ...
 * time:2018-7-9
 */
var TableInit = function(options) {
	var $table = $("#"+options.tableId);
	    var oTableInit = new Object();
	    //初始化Table
	    oTableInit.Init = function() {
	        $table.bootstrapTable({
	            url: options.tableAjax.url, //请求后台的URL（*）
	            method: options.tableAjax.type, //请求方式（*）
	            toolbar: options.toolbar, //工具按钮用哪个容器
	            striped: true, //是否显示行间隔色
	            cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
	            pagination: options.pagination==false?false:true,//是否显示分页（*）
    ...
    BootstrapTable.prototype.initServer = function (silent, query, url) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function (res) {
            	//自定义添加 wyy  如果后台出现异常 直接定位到onLoadError 方法
            	if(res.code == '1'){
